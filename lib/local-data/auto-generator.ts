...
// Genera variante ID
function generateVariantId(productIndex: number, sizeIndex: number, colorIndex: number): string {
...
function calculateVariantPrice(
  ...
  const sizeUpcharge = (STORE_CONFIG.sizePricing as Record<string, number>)[size] || 0;
  price += sizeUpcharge;
...
function getImageUrls(product: SimpleProduct, colorName: string): {
  front: string;
  back: string;
  detail?: string;
  lifestyle?: string;
...
function generateFullProduct(simpleProduct: SimpleProduct, index: number): Product {
  const { 
    title, 
    description, 
    basePrice, 
  ...
  const options = [
    ...
    { id: 'gid://option/color', name: 'Color', values: colors.map(c => c.name) }
  ];
  ...
  sizes.forEach((size, sizeIndex) => {
    colors.forEach((color, colorIndex) => {
      const variantPrice = calculateVariantPrice(basePrice, size, color.name, simpleProduct);
  ...
  const featuredImageUrl = simpleProduct.generalImages?.hero || mainImageUrls.front;
  ...
  const product: Product = {
    ...
    options,
    seo: {
      title: `${title} - ${categoryConfig.displayName}`,
...
export function getStoreStats() {
  ...
  const totalVariants = localProducts.reduce((total, product) => total + product.variants.length, 0);
  ...
  return {
    ...
    priceRange: {
      min: Math.min(...localProducts.map(p => parseFloat(p.priceRange.minVariantPrice.amount))),
      max: Math.max(...localProducts.map(p => parseFloat(p.priceRange.maxVariantPrice.amount)))
    }
  };
}