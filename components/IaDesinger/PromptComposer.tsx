import { ChevronDown, ChevronRight, Edit3, HelpCircle, MousePointer, RotateCcw, Store, Upload, Wand2 } from 'lucide-react';
import React, { useRef, useState } from 'react';
import { useAppStore } from 'storeIA/useAppStore';
import { useImageEditing, useImageGeneration } from '../../hooks/useImageGeneration';
...
export const PromptComposer: React.FC = () => {
  ...
  const tools = [
    { id: 'generate', icon: Wand2, label: 'Generar', description: 'Crear desde texto' },
    { id: 'edit', icon: Edit3, label: 'Editar', description: 'Modificar existente' },
    { id: 'mask', icon: MousePointer, label: 'Seleccionar', description: 'Clic para seleccionar' },
  ] as const;
    ...
    <div className="w-80 lg:w-72 xl:w-80 h-full bg-gray-950 border-r border-gray-800 p-6 flex flex-col space-y-6 overflow-y-auto">
      ...
      <div>
        <div>
          <label className="text-sm font-medium text-gray-300 mb-1 block">
            {selectedTool === 'generate' ? 'Imágenes de referencia' : selectedTool === 'edit' ? 'Referencias de estilo' : 'Subir imagen'}
          </label>
          {selectedTool === 'mask' && (
            <p className="text-xs text-gray-400 mb-3">Edita una imagen con máscaras</p>
          )}
          {selectedTool === 'generate' && (
            <p className="text-xs text-gray-500 mb-3">Opcional, hasta 2 imágenes</p>
          )}
          {selectedTool === 'edit' && (
            <p className="text-xs text-gray-500 mb-3">
              {canvasImage ? 'Referencias de estilo opcionales, hasta 2 imágenes' : 'Sube una imagen para editar, hasta 2 imágenes'}
            </p>
          )}
          <input
            ref={fileInputRef}
      ...
      <div>
        <label className="text-sm font-medium text-gray-300 mb-3 block">
          {selectedTool === 'generate' ? 'Describe lo que quieres crear' : 'Describe tus cambios'}
        </label>
        <Textarea
          value={currentPrompt}
        ...
        >
          ...
          <span className="text-gray-500 group-hover:text-gray-400">
            {currentPrompt.length < 20 ? 'Añade detalles para mejores resultados' :
             currentPrompt.length < 50 ? 'Buen nivel de detalle' : 'Excelente nivel de detalle'}
          </span>
        </button>
      </div>
        ...
        {showClearConfirm && (
          <div className="mt-3 p-3 bg-gray-800 rounded-lg border border-gray-700">
            <p className="text-xs text-gray-300 mb-3">
              ¿Estás seguro de que quieres limpiar esta sesión? Esto eliminará todas las subidas, prompts y contenido del canvas.
            </p>
