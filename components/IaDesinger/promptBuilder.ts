import { ProductConfig, DesignConfig, DesignPosition } from 'storeIA/useAppStore';
...
export function buildDesignPrompt(options: BuildPromptOptions): string {
  const { productConfig, designConfig, userPrompt, hasReferenceImage } = options;
 ...
 */
export function validateDesignPrompt(userPrompt: string): { valid: boolean; reason?: string } {
  const lowercasePrompt = userPrompt.toLowerCase();
  
  // Palabras prohibidas que indicarían contenido inapropiado
  const prohibitedTerms = [
    'desnudo', 'nude', 'naked', 'porn', 'xxx', 
    'violencia', 'violence', 'blood', 'gore',
    'drogas', 'drugs', 'cocaine', 'marijuana',
    'armas', 'weapons', 'gun', 'rifle',
    'hate', 'nazi', 'racial', 'discrimin',
  ];
  ...
  const warningTerms = ['foto real', 'fotografía', 'retrato realista', 'realistic photo'];
  for (const term of warningTerms) {
    if (lowercasePrompt.includes(term)) {
      return {
        valid: true,
        reason: 'Los retratos fotográficos pueden no traducirse bien a estampados. Considera un estilo más artístico.'
      };
    }
  }
