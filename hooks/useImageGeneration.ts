...
export const useImageEditing = () => {
  ...
  const editMutation = useMutation({
    mutationFn: async (instruction: string) => {
      ...
      const sourceImage = canvasImage || editReferenceImages[0];
      ...
      if (brushStrokes.length > 0) {
        ...
        brushStrokes.forEach((stroke) => {
          if (stroke.points.length >= 4) {
            ...
            ctx.beginPath();
            ctx.moveTo(stroke.points[0] || 0, stroke.points[1] || 0);

            for (let i = 2; i < stroke.points.length; i += 2) {
              ctx.lineTo(stroke.points[i] || 0, stroke.points[i + 1] || 0);
            }
            ctx.stroke();
          }
        });
        ...
        brushStrokes.forEach((stroke) => {
          if (stroke.points.length >= 4) {
            ...
            maskedCtx.beginPath();
            maskedCtx.moveTo(stroke.points[0] || 0, stroke.points[1] || 0);
      ...
      const request: EditRequest = {
        ...
        referenceImages:
          referenceImages.length > 0
            ? (referenceImages.filter(Boolean) as string[])
            : undefined,
        maskImage,
        temperature,
